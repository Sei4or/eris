...
* @prop {PrivateChannel? | TextChannel? | VoiceChannel? | NewsChannel?} channel The channel in which the interaction was created
...
* @prop {Guild?} guild The guild in which the interaction was created
* @prop {String} id The interaction id
* @prop {Member?} member the member who triggered the interaction
* @prop {User?} user the user who triggered the interaction (DMs only)
...
class Interaction extends Base {
    ...
    * @arg {Boolean} content.wait Whether to wait for the created message object
    ...
    async editParent(content) {
        if(content !== undefined) {
            if(typeof content !== "object" || content === null) {
                ...
                content.content = "" + content.content;
            } else if(content.content === undefined && !content.embeds && content.flags === undefined && content.components === undefined) {
                return Promise.reject(new Error("No content, embed or flags"));
            }
            if(content.content !== undefined || content.embed || content.allowedMentions) {
                content.allowed_mentions = this._client._formatAllowedMentions(content.allowedMentions);
